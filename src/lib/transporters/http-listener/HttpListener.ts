...
class HttpListener extends AbstractHttpListener {

    public constructor(
        private readonly _opts: dH.IHttpListenerOptions,
    ) {

        super();

        this.port = _opts.port ?? v1.DEFAULT_INSECURE_PORT;
    }
    ...
    public async start(): Promise<void> {
        ...
        const gateway = Http.createServer(this._apiProcessor);
        ...
        gateway.listen(
            this._opts.port ?? v1.DEFAULT_INSECURE_PORT,
...
export function createHttpListener(opts: dH.IHttpListenerOptions = {}): dH.IHttpListener {

    return new HttpListener(opts);
}
