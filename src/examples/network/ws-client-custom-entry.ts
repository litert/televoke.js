...
(async () => {
    ...
    const connector = WebSocket.createWsConnector({
        'port': parseInt(getClaOption('port', '25858')),
    ...
    const client: Tv.Clients.IClient<IApis> = Tv.Clients.createJsonApiClient<IApis>(connector);

    connector.setHeaders({ 'x-tv-test': 'hello-world' });
